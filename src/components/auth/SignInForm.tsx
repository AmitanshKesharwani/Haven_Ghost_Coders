...
import { detectProxyIssues } from '../../utils/proxyDetection';
...
export const SignInForm: React.FC<SignInFormProps> = ({
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      // Run network diagnostics before attempting sign in
      const proxyCheck = await detectProxyIssues();
      
      if (!proxyCheck.canReachGoogle || !proxyCheck.canReachFirebase) {
        const diagnosticMsg = `Connection issues detected:\n${proxyCheck.recommendations.join('\n')}`;
        setError('Network connectivity issues detected. Please check your proxy settings or try a different network.');
        toast.error('Network connectivity issues. Check console for details.');
        console.warn('Network Diagnostics:', diagnosticMsg);
        setLoading(false);
        return;
      }
      ...
      if (error.message?.includes('network') || 
        ...
        errorMessage = 'Network connection failed. This is likely a proxy/firewall issue. Try:\n• Disable proxy in Windows settings\n• Use a different network\n• Try incognito mode';
      }
      
      setError(errorMessage);
      toast.error('Connection failed - check proxy settings');
    } finally {
      setLoading(false);
    }
  };
